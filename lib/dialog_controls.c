...
static int static_callback(Window wID, XEvent *pEvent)
{
  ...
  {
    return 0; // not handled (use WB_POINTER messages generated by toolkit)
  }
  else if(pEvent->type == KeyPress || pEvent->type == KeyRelease)
  {
    return 0; // NOT handled (use WB_CHAR messages instead)
  }

  if(pEvent->type == ClientMessage &&
     pEvent->xclient.type == aWB_POINTER &&
     pDialogControl != NULL)
  {
...
//  int nChar = sizeof(tbuf);
  ...
  {
    // ClientMessage events - I want WB_CHAR and WB_POINTER

    if(pEvent->xclient.message_type == aWB_CHAR)
    {
    ...
    else if(pEvent->xclient.message_type == aWB_POINTER)
    {
...
//  int nChar = sizeof(tbuf);
  ...
  // TODO:  keystroke handling via WB_CHAR
...
WB_RECT rctTemp;
  ...
  { // DebugLevel_Heavy
            ...
            || iKey == 9)            // tab (TODO:  check flags to see if I should accept this)
    {
      ...
      // since the text was not changed return 0 _NOW_ so that WB_CHAR handling happens in default processing
    }
