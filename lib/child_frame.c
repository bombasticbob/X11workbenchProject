...
//          if(nChar > 0) // only for "real" characters (i.e. not just the ALT key)
      ...
      case ClientMessage:
        if(pEvent->xclient.message_type == aWB_CHAR) // generated by WBDefault
        {
          ...
          else if(WBCheckDebugLevel(DebugLevel_Heavy | DebugSubSystem_Keyboard))
          {
            WB_ERROR_PRINT("%s - WB_CHAR, no UI\n", __FUNCTION__);
        ...
        else if(pEvent->xclient.message_type == aWB_POINTER) // generated by WBDefault
        {
          ...
          {
            WB_ERROR_PRINT("%s - WB_POINTER, no UI\n", __FUNCTION__);
            WBDebugDumpEvent((XEvent *)pEvent);
          }
        }
        else if(pEvent->xclient.message_type == aSCROLL_NOTIFY) // generated by WBScrollBarEvent()
        {
          ...
          else if(WBCheckDebugLevel(DebugLevel_Heavy | DebugSubSystem_ScrollBar))
          {
            WB_ERROR_PRINT("%s - WB_SCROLL_NOTIFY, no UI\n", __FUNCTION__);
            WBDebugDumpEvent((XEvent *)pEvent);
          }
        }
     ...
     pEvent->xdestroywindow.window == wID)
  {
    ...
    if(!iRval)
    {
      WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Frame,
                   "%s - DestroyNotify *NOT* handled by superclass\n", __FUNCTION__);
    }
    ...
    if(pC && pC->wID != None) // haven't called FWDestroyChildFrame yet?
    {
      ...
      WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Frame,
                   "%s - DestroyNotify handled by Child Window base class\n", __FUNCTION__);
    }
...
//#ifndef NO_DEBUG
//  WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Mouse,
//                 "%s - WB_POINTER\n", __FUNCTION__);
//  WBDebugDumpEvent((XEvent *)pEvent);
//#endif // NO_DEBUG
  ...
  switch(pEvent->data.l[0])
  {
    ...
    default:
      ...
      if(WBCheckDebugLevel(DebugLevel_Light | DebugSubSystem_ScrollBar))
      {
        WB_ERROR_PRINT("%s.%d - unhandled mousie message\n", __FUNCTION__, __LINE__);
                                   ...
                                   Window wID, WBChildFrame *pC, WBChildFrameUI *pUI)
{
  if(pEvent->data.l[0] == WB_SCROLL_VERTICAL)
  {
    WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_ScrollBar,
                   "%s %d - SCROLL_NOTIFY - SCROLL_VERTICAL, %lx, %lx, %lx, %lx\n",
                   __FUNCTION__, __LINE__,
                   pEvent->data.l[1],
                   pEvent->data.l[2],
                   pEvent->data.l[3],
  ...
  {
    WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_ScrollBar,
                   "%s %d - SCROLL_NOTIFY - SCROLL_HORIZONTAL, %lx, %lx, %lx, %lx\n",
                   __FUNCTION__, __LINE__,
                   pEvent->data.l[1],
                   pEvent->data.l[2],
                   pEvent->data.l[3],
...
//#ifndef NO_DEBUG
//  WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Keyboard,
//                   "%s - WB_CHAR\n", __FUNCTION__);
//  WBDebugDumpEvent((XEvent *)pEvent);
//#endif // NO_DEBUG
  ...
  if(nChar > 0) // normal ASCII characters
  {
    ...
    else if(iKey == 3)  // CTRL+C
    {
      if((iACS & WB_KEYEVENT_ACSMASK) == WB_KEYEVENT_CTRL)       // control key will always be pressed, but no shift
      {
        ...
        {
          WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Event | DebugSubSystem_Clipboard | DebugSubSystem_Keyboard,
        ...
        {
          XBell(pDisplay, -100);
          WB_DEBUG_PRINT(DebugLevel_Light | DebugSubSystem_Event | DebugSubSystem_Clipboard | DebugSubSystem_Keyboard,
                         "%s.%d - no selection, can't 'COPY'\n", __FUNCTION__, __LINE__);
        }
...
#undef KEYSYM_MATCH_CURSOR_NAME
      else
      {
        ...
        else
        {
          ...
          WB_DEBUG_PRINT(DebugLevel_WARN | DebugSubSystem_Event | DebugSubSystem_Keyboard,
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            WB_DEBUG_PRINT(DebugLevel_Heavy | DebugSubSystem_Clipboard,
                           "%s - no selection or is empty\n", __FUNCTION__);
