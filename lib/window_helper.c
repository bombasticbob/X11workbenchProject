...
/**********************************************************************/
/** \ingroup core
  * \defgroup xatoms ClientMessage Atoms
  *
...
/** \ingroup xatoms
  ...
  * TODO document RESIZE_NOTIFY format here
*/
...
/** \ingroup xatoms
  ...
  * via ClientMessage passed to the appropriate window in addition to normal
  * pointer notification events.
  *
...
/** \ingroup core
  * \defgroup wmatom Window Manager Atoms */
...
//  aPRIMARY          = XInternAtom(pDisplay, "PRIMARY", False);
//  aSECONDARY        = XInternAtom(pDisplay, "SECONDARY", False);
...
//  aWINDOW           = XInternAtom(pDisplay, "WINDOW", False);
//  aBITMAP           = XInternAtom(pDisplay, "BITMAP", False);
//  aDRAWABLE         = XInternAtom(pDisplay, "DRAWABLE", False);
//  aCOLORMAP         = XInternAtom(pDisplay, "COLORMAP", False);
  aPIXEL            = XInternAtom(pDisplay, "PIXEL", False);
//  aPIXMAP           = XInternAtom(pDisplay, "PIXMAP", False);
  aTEXT             = XInternAtom(pDisplay, "TEXT", False);
//  aSTRING           = XInternAtom(pDisplay, "STRING", False);
...
#if 0
  WB_ERROR_PRINT("%s - TEMPORARY - atom list (default)\n"
                 "    pDisplay=%p\n"
                 "        aINCR=%d\n"
                 "        aCLIPBOARD=%d\n"
                 "        aTEXT=%d\n"
                 ...
                 aINCR, aCLIPBOARD, aTEXT, aC_STRING, aCOMPOUND_TEXT,
                 aTARGETS, aMULTIPLE, aTIMESTAMP, aPIXEL,
#ifdef X_HAVE_UTF8_STRING /* this indicates the extension is present */
                 aUTF8_STRING,
#endif // X_HAVE_UTF8_STRING
                 aNULL);
  ...
  // set an error handler that will continue execution after printing an appropriate message
  // in lieu of basically KILLING THE APP after some trivial B.S. "error"
...
void WBGetClientRect(Window wID, WB_RECT *pRect)
{
  ...
  pRect->left = geom.border; // 0
  pRect->top = geom.border; // 0
